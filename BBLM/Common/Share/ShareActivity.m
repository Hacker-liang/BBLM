...
@property (nonatomic, strong) UIViewController  *containerCtl;
...
- (id)initWithShareTitle:(NSString *)title andShareContent:(NSString *)content shareUrl:(NSString *)url shareImage:(UIImage *)image shareImageUrl:(NSString *)imageUrl
{
    if (self = [super init]) {
        ...
        [self creatButtonsWithTitle:@"分享至" cancelButtonTitle:@"取消" shareButtonTitles:@[@"朋友圈", @"微信"] withShareButtonImagesName:@[@"ic_sns_pengyouquan", @"ic_sns_weixin"]];
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    ...
    self.backGroundView.backgroundColor = [UIColor whiteColor];//[APP_PAGE_COLOR colorWithAlphaComponent:0.9];
...
- (UIButton *)creatCancelButtonWith:(NSString *)cancelButtonTitle
{
    ...
    [cancelButton setTitleColor:[[UIColor redColor] colorWithAlphaComponent:0.4] forState:UIControlStateHighlighted];
...
- (void)didClickOnImageIndex:(UIButton *)button
{
    ...
    if (button.tag == 0) {
        ...
        [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatTimeline] content:title image:_shareImage location:nil urlResource:resource presentedController:self.containerCtl completion:^(UMSocialResponseEntity *response){
        ...
        [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatSession] content:_shareContent image:_shareImage location:nil urlResource:resource presentedController:nil completion:^(UMSocialResponseEntity *response){
            if (response.responseCode == UMSResponseCodeSuccess) {
                NSString *url = [NSString stringWithFormat:@"%@barbie/edit", BASE_API];
                [LMNetworking GET:url parameters:@{@"share": [NSNumber numberWithInteger:1], @"id": [NSNumber numberWithInteger:[LMAccountManager shareInstance].account.userId]} success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
                    if ([[responseObject objectForKey:@"code"] integerValue] == 0) {
