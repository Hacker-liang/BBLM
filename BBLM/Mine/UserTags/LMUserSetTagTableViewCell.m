...
@implementation LMUserSetTagTableViewCell
...
+ (CGFloat)heigthOfCellWithDataSource:(NSArray *)dataSource
{
    ...
    for (int j=0; j < dataSource.count; j++) {
        ...
        itemSize = CGSizeMake(itemSize.width+50, 45);
...
- (void)awakeFromNib {
    ...
    [_collectionView registerNib:[UINib nibWithNibName:@"LMUserTagCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"grabTagCollectionCell"];
...
- (CGSize)collectionView:(UICollectionView *)collectionView sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return CGSizeMake(size.width+40, 45);
}
